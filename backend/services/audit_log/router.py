...
router = APIRouter(prefix="/api/tickets", tags=["Audit Log / Tickets"])
...
async def audit_log_lifespan(app):
    """
    Lifespan context manager for the Audit Log module.

    Usage in the main FastAPI app:
        from sahayak.backend.services.audit_log.router import router as audit_log_router, audit_log_lifespan

        app = FastAPI(lifespan=audit_log_lifespan)
...
async def create_ticket(request: TicketCreateRequest):
    """
    Internal endpoint to ingest events and create tickets.
    """
...
@router.post("/{ticket_id}/revert", response_model=TicketResponse)
...
@router.post("/{ticket_id}/inquiries", response_model=TicketResponse)
...
@router.patch("/{ticket_id}/inquiries/{inquiry_id}/answer", response_model=TicketResponse)
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            # We don't expect messages from the client in this unidirectional broadcast setup,
